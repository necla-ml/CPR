...
def main_Kinetics400(mode, k400_path, f_root, csv_root='../data/kinetics400'):
    ...
    if not os.path.exists(csv_root): os.makedirs(csv_root)
    if mode == 'train':
        train_split = get_split(os.path.join(f_root), train_split_path, 'train')
        write_list(train_split, os.path.join(csv_root, 'train_split_frame.csv'))
    elif mode == 'val':
...
if __name__ == '__main__':
    ...
    main_Kinetics400(mode='train', # train or val or test
                     k400_path='kinetics400',
                     f_root='kinetics400/frame/train_256',
                     csv_root='kinetics400')