...
def parse_args():
    ...
    parser.add_argument('--ratio', default=0.5, type=float,
                        help='selection ratio for k-nn (default: 0.5)')
    parser.add_argument('--stage', default=7, type=int,
                        help='number of cascade stage for k-nn (default: 7)')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.model == 'coclr':
        model = CoCLR(args.net, args.moco_dim, args.moco_k, args.moco_m, args.moco_t, topk=args.topk, ratio=args.ratio, stage=args.stage, reverse=args.reverse, cascade=args.cascade)
        if args.reverse:
    ...
    writer_train = SummaryWriter(log_dir=os.path.join(args.img_path, 'train'))
...
def set_path(args):
    if args.resume: exp_path = os.path.dirname(os.path.dirname(args.resume))
        ...
        exp_path = 'ckpt/log-{args.prefix}/{args.name_prefix}{args.model}-top{args.topk}{0}_k{args.moco_k}_{args.dataset}-{args.img_dim}_{args.net}_\
